               ...
               ->  Nested Loop  (cost=0.11..1082.67 rows=6 width=79) (actual time=678.029..22358.070 rows=15007 loops=1)
                     ...
                     ->  Nested Loop  (cost=0.10..1082.13 rows=6 width=71) (actual time=677.959..20082.161 rows=2996 loops=1)
                           ...
                           ->  Nested Loop  (cost=0.09..1082.04 rows=20 width=75) (actual time=326.444..20068.404 rows=3519 loops=1)
                                 ...
                                 ->  Index Scan using info_type_pkey on info_type it2  (cost=0.00..9.21 rows=1 width=4) (actual time=0.025..0.035 rows=1 loops=1)
                                 ...
                                 ->  Nested Loop  (cost=0.09..1072.84 rows=20 width=71) (actual time=326.416..20066.739 rows=3519 loops=1)
                                       ...
                                       ->  Nested Loop  (cost=0.07..931.65 rows=49 width=46) (actual time=229.484..6614.870 rows=7279 loops=1)
                                             ...
                                             ->  Nested Loop  (cost=0.07..930.89 rows=172 width=50) (actual time=229.466..6584.395 rows=7279 loops=1)
                                                               ...
                                                               ->  Nested Loop  (cost=0.04..13743.70 rows=15826 width=33) (actual time=62.492..3709.236 rows=20259 loops=1)
                                                                     ...
                                                                     ->  Nested Loop  (cost=0.03..13218.84 rows=29462 width=18) (actual time=0.036..558.675 rows=47762 loops=1)
                                                                           ...
                                                                           ->  Nested Loop  (cost=0.02..12346.83 rows=12213 width=10) (actual time=0.029..206.863 rows=21301 loops=1)
                                                                                 ...
                                                                                 ->  Index Scan using movie_id_movie_info_idx on movie_info_idx mi_idx1  (cost=0.01..12031.00 rows=1380035 width=14) (actual time=0.008..133.257 rows=63903 loops=1)
                                                                                       Buffers: shared hit=273 read=553
                                                                                 ->  Materialize  (cost=0.00..6.13 rows=1 width=4) (actual time=0.000..0.000 rows=1 loops=63903)
                                                                                       Buffers: shared hit=1 read=1
                                                                                       ->  Index Scan using info_type_info_key on info_type it1  (cost=0.00..6.13 rows=1 width=4) (actual time=0.012..0.013 rows=1 loops=1)
                                                                                             Index Cond: ((info)::text = 'rating'::text)
                                                                                             Buffers: shared hit=1 read=1
                                                                           ->  Index Scan using movie_id_movie_companies on movie_companies mc1  (cost=0.01..0.07 rows=2 width=8) (actual time=0.011..0.014 rows=2 loops=21301)
                                                                                 Index Cond: (movie_id = mi_idx1.movie_id)
                                                                                 Buffers: shared hit=88125 read=907
                                                                     ->  Index Scan using company_name_pkey on company_name cn1  (cost=0.01..0.02 rows=1 width=23) (actual time=0.065..0.065 rows=0 loops=47762)
                                                   ...
                                                   ->  Index Scan using title_idx_id on title t1  (cost=0.01..0.06 rows=1 width=25) (actual time=0.330..0.330 rows=1 loops=7279)
                                                         Index Cond: (id = mc1.movie_id)
                                                         Buffers: shared hit=28740 read=376
                                             ->  Index Scan using kind_type_pkey on kind_type kt1  (cost=0.00..0.00 rows=1 width=4) (actual time=0.003..0.003 rows=1 loops=7279)
                                                   Index Cond: (id = t1.kind_id)
                                                   Filter: ((kind)::text = ANY ('{"tv series",episode}'::text[]))
                                                   Buffers: shared hit=14558
                                       ->  Index Scan using title_idx_id on title t2  (cost=0.01..2.88 rows=1 width=25) (actual time=1.847..1.847 rows=0 loops=7279)
                                             Index Cond: (id = ml.linked_movie_id)
                                             Filter: ((production_year >= 2000) AND (production_year <= 2010))
                                             Rows Removed by Filter: 1
                                             Buffers: shared hit=28113 read=1028
                           ->  Index Scan using kind_type_pkey on kind_type kt2  (cost=0.00..0.00 rows=1 width=4) (actual time=0.003..0.003 rows=1 loops=3519)
                                 Index Cond: (id = t2.kind_id)
                                 Filter: ((kind)::text = ANY ('{"tv series",episode}'::text[]))
                                 Rows Removed by Filter: 0
                                 Buffers: shared hit=7038
                     ->  Index Scan using movie_id_movie_companies on movie_companies mc2  (cost=0.01..0.09 rows=2 width=8) (actual time=0.510..0.757 rows=5 loops=2996)
                           Index Cond: (movie_id = t2.id)
                           Buffers: shared hit=14312 read=176
               ->  Index Scan using company_name_pkey on company_name cn2  (cost=0.01..0.02 rows=1 width=23) (actual time=0.012..0.012 rows=1 loops=15007)
                     Index Cond: (id = mc2.company_id)
                     Buffers: shared hit=59978 read=50
         ->  Index Scan using movie_id_movie_info_idx on movie_info_idx mi_idx2  (cost=0.01..0.03 rows=2 width=14) (actual time=0.111..0.112 rows=1 loops=15007)
